...
from wgan import wgangp_optimizer, DataReader
...
parser.add_argument('-d','--dataset', action='store', 
    dest='dataset', required = False, default=os.getcwd(),
    help = "dataset")
...
try:
    ...
    # Read dataframe
    reader          = DataReader( args.dataset, source='raw'  )
    training_data   = reader( 'train', test=test, sort=sort)
    validation_data = reader( 'val'  , test=test, sort=sort)
